...
```bash
python3 paper-code/main.py --new-experiment
```
...
3) Container and chunks
   ...
   - Each chunk stores (a) any needed model state, (b) the compressed range-coded stream, and (c) metadata.
...
Why a GPU build matters
- The neural predictor typically dominates runtime for strong models; GPUs drastically reduce inference and training time.
- Even if the range coder itself remains on CPU, pushing probability computation to GPU yields large end-to-end wins when chunked and pipelined.

Suggested tuning order
1) Confirm correctness with smaller `seq_len` and a tiny dataset
2) Enable GPU + fp16 and benchmark end-to-end
3) Sweep `chunks_count` to balance parallelism vs overhead
4) Increase `batch_size` or effective parallelism until utilization is high without OOM
